  ...
  */
		...
		value: function getElementsByAttributes(oAttributes) {
			...
			var _loop = function _loop() {
				...
				var bMatch = undefined === Object.keys(oAttributes).find(function (sAttributeName) {
					return oCurrent.getAttribute(sAttributeName) !== oAttributes[sAttributeName];
				});
				if (bMatch === true) {
   ...
   */
		...
		value: function destroy() {
			var aVisitStack = [this];
			while (aVisitStack.length !== 0) {
				var _oCurrent = aVisitStack[0];

				aVisitStack.shift();
				var _aChilds = _oCurrent.getChildren();
				for (var i = 0; i < _aChilds.length; i++) {
					aVisitStack.push(_aChilds[i]);
				}

				if (this._deleted !== true) {
					_oCurrent._deleted = true;
					_oCurrent.removeFromParent();

					_oCurrent.oAttributes = undefined;
					_oCurrent.aChildNodes = undefined;
					_oCurrent.oParentNode = undefined;
